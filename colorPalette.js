/* eslint-disable */
    var color = {
      ...
      function getPixel (bitmapData, x, y) {
        ...
        return {r : bitmapData._data.data[i], g : bitmapData._data.data[i+1], b : bitmapData._data.data[i+2], a : bitmapData._data.data[i+3]} ;
      }
...
function ColorToHex(color){
	var res = Math.round(color * 255).toString(16);
...
jimp.read(f, function (err, image){
	if (err){
		console.log(err)
	} else {
		extract(image.bitmap, image.bitmap.width, image.bitmap.height, count, mode);
	}
});
